...
class OriginalBoard(Wall):
    def __init__(self,mode,x,y,width,height):
        ...
        bottomWall=Wall(2*self.x,mode.height-3*self.y,mode.width-4*self.width,\
            self.height)
        #bottom right
        bottomRight=Wall(2*self.x,2*self.y+0.6*(mode.height-4*self.height),self.width,\
            0.4*(mode.height-4*self.height))
        #top right
        topRight=Wall(2*self.x,2*self.y,self.width,\
            0.4*(mode.height-4*self.height))
        #bottom left
        bottomLeft=Wall(mode.width-3*self.x,2*self.y+0.6*(mode.height-4*self.height),self.width,\
            0.4*(mode.height-4*self.height))
        #top left
        topLeft=Wall(mode.width-3*self.x,2*self.y,self.width,\
            0.4*(mode.height-4*self.height))
        #1
        ...
        self.board=[wall11,wall51,wall52,wall53,wall54,wallDash,wall12,wall13,\
            wall21,wall22,wall23,wall24,bottomRight,topRight,bottomLeft,\
                topLeft,topWall,bottomWall]
        self.dimensions=set()
    ...
    def drawCells(self,mode):
        self.rows=(mode.height)//50
        self.cols=(mode.width)//50
